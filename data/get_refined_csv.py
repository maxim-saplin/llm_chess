...
Modes (GameMode):
- RANDOM_VS_LLM: White=random, Black=LLM. Groups by Player. Writes data/refined.csv.
...
- DRAGON_VS_LLM: White=Dragon, Black=LLM. Groups by (Player, white_opponent). Writes
  data/dragon_refined.csv. Leaderboard: Dragon level ASC, then Win Rate DESC,
  then Win/Loss DESC.
- ELO: Combines Random-vs-LLM and Dragon-vs-LLM to estimate Elo per Player and prints an Elo
  leaderboard. Writes data/elo_refined.csv.
...
Pricing & tokens: model prices from data/models_metadata.csv (per 1M tokens). If
...
FILTER_OUT_MODELS = [
    ...
    "gpt-6-astra-2026-09-03-xhigh", # too few
...
def print_metrics_explanation():
    """Explain the metrics shown by the leaderboard tables."""
    print("\nMETRICS EXPLANATION:")
    print("- Elo: Estimated playing strength relative to the calibrated Random/Dragon opponents.")
    print("- Win Rate: Percentage of games won by the LLM (it plays Black); draws are not wins.")
    print(
        "- Win/Loss score: 0.5 + (wins - losses) / (2 * games). "
        "0.50 is neutral, 1.00 is all wins, and 0.00 is all losses."
    )
    print(
        "- Game Duration: Average completion score per game. A non-interrupted game contributes 100%; "
        "an interrupted game contributes recorded turns / 200."
    )
    print(
        "- Tokens (completion_tokens_black_per_move): Total API-reported output tokens generated by Black "
        "divided by total recorded turns across the games."
    )
    print(
        "  This is a per-turn (per ply) average, not tokens per game. number_of_moves counts both White "
        "and Black turns, so it is not strictly tokens per Black reply; prompt/input tokens are excluded."
    )
    print("- Cost/Game: Average API cost for one game, including prompt and completion tokens.")
    print("- Cost/Move: Average API cost for one recorded turn, derived from the per-game costs.")
    print("- Time/Game: Average accumulated model reply time per game when timing data is available.")
    print("- Games: Number of games included after the configured filters.")
    print("- Total Cost: Cost/Game multiplied by Games for that model.")
    print("- ± values: Approximate 95% margin of error for the corresponding metric.")
