...
def aggregate_models_to_csv(
    ...
    select_only_n=30,  # If it exists, will stop after the first n rows from the aggregate file. This is to ensure we can equally compare the results
    filter_out_errors: bool = True, # If True, will filter out games with errors
) -> None:
    """
    ...
    headers = [
        ...
        # white and black checkmates
        "white_rand_checkmates",
        "black_llm_checkmates",
    ]
    ...
    for model_name, model_logs in model_groups.items():
        ...
        if filter_out_errors:
            ...
            model_logs = [
                log for log in model_logs if not log.reason in { TerminationReason.ERROR.value, TerminationReason.UNKNOWN_ISSUE.value }
            ]
            print(f"Filtered count: {len(model_logs)}")
        if select_only_n and len(model_logs) > select_only_n:
            print(f"Limiting to the first {select_only_n} logs for aggregation.")
            model_logs = model_logs[:select_only_n]
        ...
        # Count number of checkmates by either player
        white_rand_checkmates = sum(
            1 for log in model_logs if log.reason == TerminationReason.CHECKMATE.value
            and log.winner == "Random_Player"
        )
        black_llm_checkmates = sum(
            1 for log in model_logs if log.reason == TerminationReason.CHECKMATE.value
            and log.winner == "Player_Black"
        )
        ...
        csv_data.append(
            [
                ...
                std_dev_game_cost,
                moe_average_game_cost,
                # Add white and black checkmates
                white_rand_checkmates,
                black_llm_checkmates,
            ]
        )
